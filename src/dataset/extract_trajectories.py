...
def compute_direct_flows_for_start_frame(
    ...
    h, w = load_image(images[0], device).shape[2:4] if infer_res_size is None else infer_res_size
    video = torch.cat(
        [load_image(imfile, device) for imfile in images], dim=0
    )  # (video_len - starting_frame) x 3 x h x w
    upper_bound = torch.tensor([[w, h]]).to(device) - 1
    ...
    coords1_backward = bilinear_interpolate_video(
        rearrange(backward_flows, "t c h w -> 1 c t h w"),
        rearrange(coords1_3d, "t h w d -> (t h w) d"),
        h=h,
        w=w,
        t=coords.shape[0],
        normalize_h=True,
...
def save_trajectories(args):
    ...
    fflow, bflow = (
        flows[0:1],
        flows[1:2],
    )  # forward and backward flows, 1 x (t-1) x 2 x h x w
    ...
    for starting_frame in tqdm(range(t - (min_trajectory_length - 1)), leave=False):
        trajectories = (
            torch.zeros((t - starting_frame, h, w, 2)).float().to(device)
        )  # all trajectories starting from starting_frame, (t) x h x w x 2
        coords = rearrange(coords_grid(1, h, w, device=device), "d l h w -> d h w l", d=1, l=2)  # h x w x 2
        orig_coords = rearrange(
            coords_grid(1, h, w, device=device), "d l h w -> d h w l", d=1, l=2
        )  # h x w x 2, for direct flow
        ...
        if look_behind:
            ...
            not_passed_through[past_passed[:, 1], past_passed[:, 0]] = (
                False  # set to False all locations that have existing trajectories passing through them.
            )
            mask |= not_passed_through

        trajectories[0] = torch.where(
            mask, coords.double(), float("nan")
        )  # set trajectories[0] to nan in locations that already have existing trajectories (starting at previous frames) passing through them, t x h x w x 2
        ...
        for idx in tqdm(range(t - 1 - starting_frame), leave=False):
            ...
            if filter_using_direct_flow:
                ...
                err_dflow = (
                    err_dflow * (dflow_mask > 0.2).float()
                )  # filter out locations where direct flow is not reliable
                mask = mask & (err_dflow.unsqueeze(-1) < direct_flow_threshold)
            trajectories[idx + 1] = torch.where(
                mask, coords.double(), float("nan")
            )  # set trajectories[idx+1] to nan in locations where the trajectory don't exist anymore (due to cycle-consistency error).
        ...
        current_not_nan_traj = padded_trajectories[
            padded_trajectories.cpu().isnan().any(dim=-1).logical_not().sum(dim=-1).cuda() >= min_trajectory_length
        ]
        all_filtered_trajectories = torch.cat(
            [all_filtered_trajectories, current_not_nan_traj.cpu()], dim=0
        )  # (N x t x 2), (M x t x 2) -> ((M+N) x t x 2)
...
#     parser.add_argument(
#         "--filter-using-direct-flow", action="store_true", default=False, help="Filter using direct flow"
#     )
